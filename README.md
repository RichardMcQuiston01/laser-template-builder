  ...
  return (
    <>
      <TemplateHeader
        ...
        onConvertToImage={async () => {
          await fetch(`/api/templates/${template.id}/convert-to-image`, {
            method: "POST",
          });
        }}
      />
...
const output = applySubstitution(svgString, template.variables, {
  "{{name}}": "Alice",
});
```
...
import {
  ...
  extractSvgTokens,
  renderSvgFile,
  validateLightBurnXml,
  extractLightBurnTokens,
  renderLightBurnFile,
  validateXcs,
  readXcsFile,
  extractXcsTokens,
